...
import { runPlanForDate } from './plan'
...
export function RunPage() {
  ...
  const [month,setMonth]=useState(new Date()), [selected,setSelected]=useState(today()), [open,setOpen]=useState(false)
  const log=runLogs[selected], plan=runPlanForDate(selected)
  ...
  return <div className="page-stack viewport-tight">
    <section className="page-heading"><div><p className="eyebrow">Run · {format(parseDate(selected), 'MMM d')}</p><h1>Running</h1><p className="subtle">{plan.scheduled ? `${plan.distanceKm} km planned · ${plan.structure}` : plan.structure}</p></div><Button onClick={()=>{setSelected(today());setOpen(true)}}><Plus size={15}/> Log today</Button></section>
    ...
    <div className="selected-day-card"><div><span>Selected day</span><strong>{format(parseDate(selected), 'EEEE, MMM d')}</strong></div><p>{log?.status==='run' ? `${log.distanceKm ?? 0} km · ${formatDuration(log.durationSeconds)} · ${formatPace((log.durationSeconds??0)/(log.distanceKm??1))}` : log?.status === 'rest' ? 'Rest day' : log?.status === 'no' ? 'Missed run' : 'Not logged'}</p></div>
    <RunDialog open={open && !isFutureDate(selected)} onClose={()=>setOpen(false)} date={selected} initial={log} onSave={async data=>{await upsertRun({...data,date:selected,updatedAt:new Date().toISOString(),schemaVersion:1});setOpen(false);toast.success('Run activity saved')}}/>
  </div>
}
function RunDialog({open,onClose,date,initial,onSave}:{open:boolean;onClose:()=>void;date:string;initial?:RunLog;onSave:(data: Omit<RunLog,'date'|'updatedAt'|'schemaVersion'>)=>Promise<void>}){
  ...
  return <Dialog open={open} onClose={onClose} title={`Run · ${format(parseDate(date),'EEE, MMM d')}`} description={runPlanForDate(date).scheduled ? `${runPlanForDate(date).distanceKm} km planned · ${runPlanForDate(date).structure}` : 'No run planned'}>
