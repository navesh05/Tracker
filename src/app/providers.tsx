...
export function AppProviders({ children }: { children: ReactNode }) {
  const theme = useAppStore(s => s.theme), hydrate = useAppStore(s => s.hydrate), setSyncState = useAppStore(s => s.setSyncState), refresh = useAppStore(s => s.refreshFromRepository), watchAuthUser = useAppStore(s => s.watchAuthUser)
  useEffect(() => { void hydrate() }, [hydrate])
  useEffect(() => { watchAuthUser() }, [watchAuthUser])
