...
export function ActivityCalendar({ month, setMonth, selectedDate, onSelect, getState }: { month: Date; setMonth: (date: Date) => void; selectedDate?: string; onSelect: (date: string) => void; getState: (date: string) => HeatState | string }) {
  ...
  return <div className="calendar">
    <div className="calendar-head"><div><strong>{monthTitle(month)}</strong><span></span></div><div className="calendar-nav"><Button variant="ghost" size="icon" onClick={() => setMonth(previousMonth(month))} aria-label="Previous month"><ChevronLeft size={16}/></Button><Button variant="ghost" size="icon" onClick={() => setMonth(nextMonth(month))} aria-label="Next month"><ChevronRight size={16}/></Button></div></div>
