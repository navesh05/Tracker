...
import { dailyLogSchema, gymLogSchema, profileSchema, runLogSchema, weightCheckInSchema } from '@/core/domain/schemas'
import type { DailyLog, GymLog, RunLog, RepositorySnapshot, SyncEntity, SyncQueueItem, SyncState, UserProfile, WeightCheckIn } from '@/core/domain/types'
...
export const defaultProfile: UserProfile = { name: 'Navesh', startWeightKg: 90, currentWeightKg: 90, targetWeightKg: 76, calorieTarget: 2297, proteinTargetG: 180, sleepTargetHours: 7.5, stepsTarget: 10000, waterTargetLiters: 3 }
...
export const localRepository: TrackerRepository = {
  async getSnapshot() {
    ...
    return {
      ...
      dailyLogs: await db.dailyLogs.orderBy('date').toArray(),
  ...
  async saveDailyLog(log) {
    const safe = dailyLogSchema.parse(log)
    await db.transaction('rw', db.dailyLogs, db.syncQueue, async () => { await db.dailyLogs.put(safe); await enqueue('daily', safe.date, safe, safe.updatedAt) })
  },
  async saveWeightCheckIn(log) {
    const safe = weightCheckInSchema.parse(log)
    assertNotFuture(safe.measuredDate)
    const profile = (await db.profile.get('profile')) ?? defaultProfile
    const nextProfile = { ...profile, currentWeightKg: safe.weightKg }
    await db.transaction('rw', db.weightCheckIns, db.profile, db.syncQueue, async () => {
      await db.weightCheckIns.put(safe)
      await db.profile.put({ ...nextProfile, id: 'profile' })
      await enqueue('weight', safe.id, safe, safe.updatedAt)
      await enqueue('profile', 'profile', { ...nextProfile }, safe.updatedAt)
    })
  },
  async mergeRemoteSnapshot(snapshot) {
    ...
    const dailies = snapshot.dailyLogs.map(x => dailyLogSchema.parse(x))
    const weights = (snapshot.weightCheckIns ?? []).map(x => weightCheckInSchema.parse(x))
    await db.transaction('rw', db.profile, db.gymLogs, db.runLogs, db.dailyLogs, db.weightCheckIns, async () => {
      ...
      for (const remote of dailies) { const local = await db.dailyLogs.get(remote.date); if (!local || newer(remote.updatedAt, local.updatedAt)) await db.dailyLogs.put(remote) }
