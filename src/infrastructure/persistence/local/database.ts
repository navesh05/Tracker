...
import type { GymLog, RunLog, SyncQueueItem, UserProfile, WeightCheckIn } from '@/core/domain/types'
...
export class TrackerDatabase extends Dexie {
  ...
  constructor() {
    ...
    // v5: Daily nutrition/recovery tracking and the week-numbered plan system were both
    // retired. Weight check-ins are now plain date + weight entries with no week concept,
    // and workout focus types moved into the user's editable profile.
    this.version(5).stores({
      gymLogs: 'date, updatedAt', runLogs: 'date, updatedAt', dailyLogs: null,
      weightCheckIns: 'id, measuredDate, updatedAt', profile: 'id',
      syncQueue: 'id, entity, entityId, createdAt, nextAttemptAt', meta: 'key',
    })
  }
}
